...
class ArtistsGraph extends Component {
  ...
  async componentDidUpdate() {
    ...
    if (previousArtists.length !== currentArtists.length) {
  ...
  render() {
    return (
      <React.Fragment>
        <div style={{ width: '100%', height: 600 }} ref={this.appRef}></div>
  ...
  addArtist = async (artist) => {
    ...
    this.setState({ artists: previousArtists, movies: currentMovies, data: currentData });
  }
